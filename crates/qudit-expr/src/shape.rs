    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GenerationShape::Tensor3D(nmats, nrows, ncols) => {
                write!(f, "Tensor3D({}, {}, {})", nmats, nrows, ncols)
            }
            GenerationShape::Tensor4D(ntens, nmats, nrows, ncols) => {
                write!(f, "Tensor4D({}, {}, {}, {})", ntens, nmats, nrows, ncols)
            }
        }
    }
}
...
impl GenerationShape {
    ...
    pub fn gradient_shape(&self, num_params: usize) -> Self {
        match self {
            ...
            GenerationShape::Matrix(nrows, ncols) => {
                GenerationShape::Tensor3D(num_params, *nrows, *ncols)
            }
            GenerationShape::Tensor3D(nmats, nrows, ncols) => {
                GenerationShape::Tensor4D(num_params, *nmats, *nrows, *ncols)
            }
            GenerationShape::Tensor4D(_, _, _, _) => {
                panic!("Unable to find shape for gradient of 4D tensor.")
            }
        }
    }
    ...
    pub fn hessian_shape(&self, num_params: usize) -> Self {
        ...
        match self {
            ...
            GenerationShape::Matrix(nrows, ncols) => {
                GenerationShape::Tensor3D(sym_sq_size, *nrows, *ncols)
            }
            GenerationShape::Tensor3D(nmats, nrows, ncols) => {
                GenerationShape::Tensor4D(sym_sq_size, *nmats, *nrows, *ncols)
            }
            GenerationShape::Tensor4D(_, _, _, _) => {
                panic!("Unable to find shape for Hessian of 4D tensor.")
            }
        }
    }
    ...
    pub fn is_1d(&self) -> bool {
        match self {
            ...
            GenerationShape::Tensor4D(ntens, nmats, nrows, ncols) => {
                let non_one_count = [*ntens, *nmats, *nrows, *ncols]
                    .iter()
                    .filter(|&&d| d > 1)
                    .count();
                non_one_count == 1
    ...
    pub fn is_3d(&self) -> bool {
        match self {
            ...
            GenerationShape::Tensor4D(ntens, _, _, _) => *ntens == 1,
        }
    }
    ...
    pub fn calculate_directions(&self, index_sizes: &[usize]) -> Vec<IndexDirection> {
        match self {
            GenerationShape::Scalar => vec![],
            GenerationShape::Vector(_) => vec![IndexDirection::Input; index_sizes.len()],
...
impl Add for GenerationShape {
    ...
    fn add(self, other: Self) -> Self::Output {
        match (self, other) {
            ...
            (
                GenerationShape::Matrix(s_nrows, s_ncols),
                GenerationShape::Matrix(o_nrows, o_ncols),
            ) => GenerationShape::Matrix(s_nrows + o_nrows, s_ncols + o_ncols),
            (
                GenerationShape::Tensor3D(s_nmats, s_nrows, s_ncols),
                GenerationShape::Tensor3D(o_nmats, o_nrows, o_ncols),
            ) => GenerationShape::Tensor3D(s_nmats + o_nmats, s_nrows + o_nrows, s_ncols + o_ncols),
            (
                GenerationShape::Tensor4D(s_ntens, s_nmats, s_nrows, s_ncols),
                GenerationShape::Tensor4D(o_ntens, o_nmats, o_nrows, o_ncols),
            ) => GenerationShape::Tensor4D(
                s_ntens + o_ntens,
                s_nmats + o_nmats,
                s_nrows + o_nrows,
                s_ncols + o_ncols,
            ...
            _ => {
                panic!("Cannot add tensors of different fundamental shapes or incompatible ranks.")
            }
        }
    }
}
