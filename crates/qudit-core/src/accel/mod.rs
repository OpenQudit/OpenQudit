...
mod macro_tests {
    ...
    use crate::{c32, c64};
    use qudit_macros::{complex_tensor32, complex_tensor64};
    ...
    fn test_nd_tensor() {
        ...
        let expected_data: Vec<c64> = (1..=2 * 3 * 3 * 3)
            .map(|i| {
                let in2 = i as f64;
        ...
        unsafe {
            assert_eq!(
                from_raw_parts(attempt.as_ptr(), 2 * 3 * 3 * 3),
                from_raw_parts(expected.as_ptr(), 2 * 3 * 3 * 3)
            );
        }
    }
    ...
    fn test_scalar_tensor() {
        ...
        unsafe {
            assert_eq!(
                from_raw_parts(attempt.as_ptr(), 1),
                from_raw_parts(expected.as_ptr(), 1)
            );
        }
    }
    ...
    fn test_tensor_with_parentheses() {
        ...
        unsafe {
            assert_eq!(
                from_raw_parts(attempt.as_ptr(), 2),
                from_raw_parts(expected.as_ptr(), 2)
            );
        }
    }
