...
use super::{instructions::{ConsecutiveParamWriteStruct, DisjointKronStruct, DisjointMatmulStruct, FRPRStruct, OverlappingKronStruct, OverlappingMatMulStruct, SplitParamWriteStruct}, SizedMatrixBuffer, SpecializedInstruction};

#[derive(Clone)]
pub enum GeneralizedInstruction {
    ConsecutiveParamWrite(TensorExpression, usize, usize),
    SplitParamWrite(TensorExpression, ParamIndices, usize),
...
impl std::fmt::Debug for GeneralizedInstruction {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            GeneralizedInstruction::ConsecutiveParamWrite(expr, _, index) => {
                write!(f, "C-Write {} {:?}", expr.name(), index)
            },
            GeneralizedInstruction::SplitParamWrite(expr, _, index) => {
                write!(f, "S-Write {} {:?}", expr.name(), index)
            },
...
impl GeneralizedInstruction {
    ...
    pub fn specialize<C: ComplexScalar>(
        ...
        match self {
            GeneralizedInstruction::ConsecutiveParamWrite(expr, start, index) => {
            ...
            GeneralizedInstruction::SplitParamWrite(expr, param_indices, index) => {
                let (utry_fn, grad_fn) = unsafe {
                    let utry_fn = module.get_function_raw(&expr.name());
                    let grad_fn = if diff_lvl != DifferentiationLevel::None {
                        Some(module.get_function_and_gradient_raw(&expr.name()))
