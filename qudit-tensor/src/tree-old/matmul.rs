...
impl MatMulNode {
    ...
    pub fn dimensions(&self) -> Vec<usize> {
        ...
        match left_shape {
            ...
            GenerationShape::Tensor3D(_, a, _) => {
        ...
        match right_shape {
            ...
            GenerationShape::Tensor3D(_, c, _) => {
                right_count_iter += 1;
                let mut prod_iter = 1;
                while prod_iter < c {
                    prod_iter *= right_dims[right_count_iter] as usize;
    ...
    pub fn generation_shape(&self) -> GenerationShape {
        ...
        match (left_shape.clone(), right_shape.clone()) {
            (GenerationShape::Matrix(l0, l1), GenerationShape::Matrix(r0, r1)) => {
                if l1 == r0 {
                    return GenerationShape::Matrix(l0, r1);
                }
            }
            (GenerationShape::Tensor3D(l0, l1, l2), GenerationShape::Tensor3D(r0, r1, r2)) => {
                if l0 == r0 && l2 == r1 {
                    return GenerationShape::Tensor3D(l0, l1, r2);
                }
            }
            _ => (),
        }
