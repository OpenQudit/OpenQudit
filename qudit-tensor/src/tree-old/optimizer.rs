...
impl TreeOptimizer {
    ...
    pub fn optimize(&self, mut tree: ExpressionTree) -> ExpressionTree {
        ...
        tree = self.merge_trace_into_leaf(tree);
    ...
    pub fn merge_trace_into_leaf(&self, tree: ExpressionTree) -> ExpressionTree {
        match tree {
            ExpressionTree::Leaf(_) => tree,
            ExpressionTree::MatMul(MatMulNode { left, right }) => {
                let left = self.merge_trace_into_leaf(*left);
                let right = self.merge_trace_into_leaf(*right);
            ...
            ExpressionTree::Transpose(TransposeNode { child, perm, dimensions, generation_shape }) => {
                let child = self.merge_trace_into_leaf(*child);
    ...
    pub fn merge_transpose_into_leaf(&self, tree: ExpressionTree) -> ExpressionTree {
        match tree {
            ...
            ExpressionTree::Transpose(TransposeNode { child, perm, dimensions, generation_shape }) => {
                if let ExpressionTree::Leaf(mut l) = *child {
                    l.permute(&perm, generation_shape);
                    ExpressionTree::Leaf(l)
                } else {
                    let child = self.merge_transpose_into_leaf(*child);
                    ExpressionTree::Transpose(TransposeNode::new(child, perm, generation_shape))
