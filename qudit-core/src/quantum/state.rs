...
#[derive(Clone)]
pub struct StateVector<R: RealScalar> {
    radices: QuditRadices,
    vector: Col<R::C>,
}

impl<R: RealScalar> StateVector<R> {
    ...
    #[inline(always)]
    #[track_caller]
    pub fn new<T: ToRadices, V: ToColVector<R::C>>(radices: T, vector: V) -> Self {
    ...
    pub fn is_pure_state<S: AsColRef<T = R::C>>(vector: S) -> bool {
        todo!()
        // let col_ref = vector.as_col_ref();
        // let norm: R = col_ref.iter().map(|c| c.abs().powi(2)).sum();
        // (norm - R::new(1.0)).abs() < R::C::THRESHOLD
    }
}

pub struct Bra<R: RealScalar> {
    inner: StateVector<R>,
}
